...
#------------------------------------------------------------------#
resource "kubernetes_secret" "ws-cert-key" {
  depends_on = [
    kubernetes_namespace.cyan
  ]
  metadata {
  ...
  data = {
    "nginx.pem" = "${file("${path.module}/myapp.pem")}"
  }
  binary_data = {
    "nginx.key" = "${filebase64("${path.module}/myapp-key.pem")}"
  }
}
...
resource "kubernetes_ingress" "tkcIngress" {
  ...
  metadata {
    ...
    annotations = {
      ...
      "acos.a10networks.com/health-monitors" = "[{\"name\":\"ws-mon\", \"port\":\"443\",\"type\":\"tcp\"}]"
      ...
      "acos.a10networks.com/virtual-ports" = "[{\"port\":\"443\",\"protocol\":\"tcp\",\"snat\":true}]"
    }
  }
  spec {
    rule {
